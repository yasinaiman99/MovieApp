...
// set up the state variables with useState hook
const MovieDetails = () => {
...
// useEffect to fetch the movie data from the API when the component is first loaded
    useEffect(() => {
        fetchData();
    }, []);
// function to fetch movie data from the API using Axios
...
// function to save the movie
    const saveShow = async () => {
        ...
        if (user?.email) {
            // update the state for like and saved
            setLike(!like)
            setSaved(true)
            ...
            // show an alert message if the user is not logged in
